    ...
    <>
      ...
      <section className="section-padding">
        <div className="container">
          ...
          <div className="grid md:grid-cols-3 gap-6 mb-16">
            {[
              ...
              <button
                ...
                className={`p-8 rounded-lg text-left transition-all duration-300 ${
                  donationType === d.type
                    ? "bg-accent/10 border-2 border-accent shadow-md"
                    : "bg-card border-2 border-border shadow-sm hover:shadow-md"
                }`}
              >
                ...
                {d.type === "monthly" && donationType === "monthly" && (
                  <span className="inline-block mt-3 label-accent text-accent bg-accent/10 px-3 py-1 rounded-md">Monthly</span>
                )}
              </button>
            ))}
          </div>
          ...
          {/* Donation Form */}
          {submitted ? (
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="max-w-xl mx-auto bg-success/10 border border-success/20 rounded-lg p-12 text-center">
              <CheckCircle size={48} className="text-success mx-auto mb-4" />
              <h3 className="text-2xl font-bold text-primary mb-2">Thank You!</h3>
              <p className="text-muted-foreground">Your donation of ${selectedAmount} will make a real difference.</p>
            </motion.div>
          ) : (
            <form onSubmit={handleSubmit} className="max-w-xl mx-auto bg-card rounded-lg shadow-md p-8 md:p-10 space-y-6 border border-border">
              <div>
                ...
                <div className="grid grid-cols-4 gap-3 mb-3">
                  {presets.map((p) => (
                    <button
                      ...
                      className={`py-3 rounded-md text-sm font-semibold transition-all duration-200 ${
              ...
              <button type="submit" className="w-full py-4 rounded-md bg-success text-success-foreground font-semibold text-base hover:bg-success/90 transition-colors active:scale-[0.98]">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {donationHelps.map((h, i) => (
              <motion.div
                ...
                className="bg-card p-6 rounded-lg shadow-sm text-center border border-border"
              >
